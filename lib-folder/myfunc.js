...
exports.tanggal = (numer = new Date()) => {
	...
	const dayName = myDays[tgl.getDay()];
...
exports.smsg = (sock, m, store) => {
	...
	m.warning = async (teks) => {
		...
		return sock.sendMessage(m.chat, nedd, {
			quoted: m,
		});
	}
	m.react = async (emoji) => {
	...
	m.sendMentions = async (teks) => sock.sendTextWithMentions(m.chat, teks, m)
